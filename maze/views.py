...
from maze.mazes_master.run import MazeRunner 
import heapq
import cv2
...
def upload(request):
    ...
    file_bytes = np.asarray(bytearray(request.FILES['img'].read()), dtype=np.uint8)
    img = cv2.imdecode(file_bytes, 1)
    #img = cv2.imdecode(np.fromstring(request.FILES['img'].read(), np.uint8), cv2.IMREAD_UNCHANGED)
    cv2.circle(img,(int(positions[0]),int(positions[1])), 3, (255,0,0), -1) # add a circle at (5, 220)
    cv2.circle(img, (int(positions[2]),int(positions[3])), 3, (0,0,255), -1) # add a circle at (5,5)
    p =find_shortest_path(img, (int(positions[0]),int(positions[1])), (int(positions[2]),int(positions[3])))
    drawPath(img,p)
    os.chdir('E:\DSIR\maze\static')
    imagename=''.join(random.choice(string.ascii_uppercase + string.digits) for _ in range(10))+'.png'
    cv2.imwrite(imagename, img)
    return HttpResponse(imagename)
def getGeneratePage(request):
    # MazeRunner(20,20)
    return render(request, 'maze/generate.html',{})
